#!/usr/bin/env bash
# check_progress.sh — gather everything needed to judge a job's ETA.
# Usage: check_progress.sh <jobid> [progress-regex]
# Prints: job state, elapsed vs walltime limit, log path/size, last 15 log
# lines, and the most recent progress-marker matches (default: "N/N" style).
set -u
