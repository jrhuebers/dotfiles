...
For every `arxiv_<id>.pdf` row in each README.md: fetch the API author list
(full names) and compare surname sets with the README's author field. Report
mismatches; never fill names in by hand (rule: metadata comes from the API,
never from memory).

Usage: python3 audit_readme_authors.py <README.md> [README2.md ...]
"""
import re
import sys
import time
...
# Match: | arxiv_<id>.pdf | <authors> | <id> |
...
def main():
    paths = sys.argv[1:]
    if not paths:
        sys.exit("usage: audit_readme_authors.py <README.md> [README2.md ...]")
    problems = 0
    for path in paths:
        try:
