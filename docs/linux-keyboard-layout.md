...
This repository provides a small user-level XKB layout for Linux desktops. The
layout is distribution-independent: it uses the standard user XKB search path,
keeps the British (`gb`) layout, and adds these level-3 mappings:
...
- `~/.config/xkb/symbols/custom` is the installed user copy or symlink of that
  file. Linux desktop stacks using `libxkbcommon`, including GNOME on Wayland,
  search user XKB configuration paths before the system layout database.
- GNOME stores the selected input sources in dconf, exposed through
  ...
  `current` selects the zero-based active source. Other desktop environments
  may use the same symbols file but require their own input-source selection
  mechanism.
...
On GNOME, select the custom layout as the only input source:
...
the shortcuts in a native Wayland application, such as GNOME Text Editor. On
another Linux distribution with GNOME/Wayland, use the same commands. On a
non-GNOME desktop, retain the symbols-file installation but select `custom`
through that desktop environment's keyboard-layout settings.
