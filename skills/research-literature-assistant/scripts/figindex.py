#!/usr/bin/env python3
"""Build a figure-caption index from the flattened .tex corpus.

For each project papers dir: parse arxiv_<id>.tex, find figure environments,
extract (source-order number, label, includegraphics target, caption text),
and write papers/FIGURES.md next to the corpus.

Captions are text, so this makes every figure searchable/quotable by the
current (text-only) LLM. The includegraphics target + src tarball path let a
future vision pass locate the actual image file.

Usage: python3 figindex.py [project_dir ...]
Defaults: FIM/papers, diffusion-qmc/papers, gauge-graph-network/papers
"""
import os
import re
import sys
...
def main():
    dirs = sys.argv[1:] if len(sys.argv) > 1 else DEFAULTS
    for pdir in dirs:
        texs = sorted(glob.glob(os.path.join(pdir, "arxiv_*.tex")))
        if not texs:
            print(f"{pdir}: no tex files, skipped")
            continue
        lines = [f"# Figures index — {os.path.basename(os.path.dirname(pdir))}",
                 "",
                 f"Generated {__import__('datetime').date.today()} from flattened "
                 ".tex sources. Figure N = source order, not LaTeX numbering. "
                 "includegraphics target + src tarball locate the actual image file.",
                 ""]
        total = nocap = noinc = 0
        for tex_path in texs:
            pid = os.path.basename(tex_path)[6:-4]  # arxiv_<id>.tex
            ...
            for i, fig in enumerate(figs, 1):
                ...
                tarball = f"src/{pid}.tar.gz -> " if os.path.exists(os.path.join(pdir, "src", f"{pid}.tar.gz")) else ""
